...
//
//  Behaviour (ported from legacy Clicky "teacher" buddy):
...
struct BlueCursorView: View {
    ...
    private static let log = Logger(subsystem: "com.proyecto26.clicky", category: "BlueCursorView")

    /// Offset applied to the mouse location so the buddy sits beside
    /// the cursor (lower-right) rather than directly on top of it.
    /// Ported from legacy Clicky's cursor-companion feel.
