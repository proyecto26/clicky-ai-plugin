...
export const OPENCLICKY_TAP = 'proyecto26/tap/openclicky';
export const OPENCLICKY_RELEASE_REPO = 'proyecto26/openclicky';
export const INSTALLED_PATH = '/Applications/OpenClicky.app';
...
export async function install(options: InstallOptions = {}): Promise<InstallResult> {
  ...
  if (options.dryRun) {
    ...
    decisions.push(isBrewAvailable() ? `brew available → would run: brew install ${OPENCLICKY_TAP}` : 'brew not available → skipping to DMG');
    decisions.push(`DMG fallback: github.com/${OPENCLICKY_RELEASE_REPO} latest → download → verify SHA256 → hdiutil attach → cp to /Applications`);
  ...
  return {
    ...
    method: 'manual',
    reason:
      `Automatic install isn't available yet (OpenClicky isn't notarized for public distribution — ` +
      `the project is open-source and not Developer-Program-enrolled). Build from source in ~2 minutes:\n` +
      `  git clone https://github.com/${OPENCLICKY_RELEASE_REPO}.git\n` +
      `  cd openclicky/native && make install\n\n` +
      `That copies OpenClicky.app into /Applications/ signed with your free Apple Development certificate.\n` +
      `Alternatively, if a pre-built DMG is attached to a release, download it at\n` +
      `https://github.com/${OPENCLICKY_RELEASE_REPO}/releases and right-click → Open to bypass Gatekeeper.\n` +
...
function tryBrewInstall(): Promise<InstallResult> {
  return new Promise((resolve) => {
    const child = spawn('brew', ['install', OPENCLICKY_TAP], { stdio: ['ignore', 'pipe', 'pipe'] });
    let stderr = '';
...
async function tryDmgInstall(): Promise<InstallResult> {
  try {
    ...
    const dmgAsset = release.assets.find(
      (a) => /OpenClicky-.*-arm64\.dmg$/.test(a.name) && !a.name.endsWith('.sha256'),
    );
    ...
    if (!existsSync(INSTALLED_PATH)) {
      return { ok: false, method: 'dmg-download', reason: 'DMG processed but OpenClicky.app not present in /Applications/' };
    }
...
async function fetchLatestRelease(): Promise<GithubRelease> {
  const url = `https://api.github.com/repos/${OPENCLICKY_RELEASE_REPO}/releases/latest`;
  const res = await fetch(url, { headers: { accept: 'application/vnd.github+json' } });
  if (!res.ok) throw new Error(`GitHub releases HTTP ${res.status} for ${OPENCLICKY_RELEASE_REPO}`);
