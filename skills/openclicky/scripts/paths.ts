...
const DATA_DIR_NAME = 'openclicky';
...
export function assertDarwin(): void {
  if (!isDarwin()) {
    throw new Error('openclicky requires macOS. Non-darwin platforms are not supported in v1.');
  }
}
...
export function resolveDataDir(): string {
  const override = process.env.OPENCLICKY_DATA_DIR?.trim();
