...
function pickAutoEngine(): SpeakEngine {
  if (process.env.OPENCLICKY_VIBEVOICE_URL?.trim()) return 'vibevoice';
  if (process.env.OPENCLICKY_ELEVENLABS_API_KEY?.trim()) return 'elevenlabs';
  return 'say';
}
...
async function speakViaVibeVoice(text: string): Promise<void> {
  const base = process.env.OPENCLICKY_VIBEVOICE_URL?.trim();
  if (!base) throw new Error('OPENCLICKY_VIBEVOICE_URL not set');
  const url = new URL('/v1/synthesize', base).toString();

  const res = await fetch(url, {
    method: 'POST',
    headers: { 'content-type': 'application/json', accept: 'audio/wav' },
    body: JSON.stringify({ text, speaker: process.env.OPENCLICKY_VIBEVOICE_SPEAKER ?? 'Carter' }),
  });
...
async function speakViaElevenLabs(text: string): Promise<void> {
  const apiKey = process.env.OPENCLICKY_ELEVENLABS_API_KEY?.trim();
  const voiceId = process.env.OPENCLICKY_ELEVENLABS_VOICE_ID?.trim() ?? 'kPzsL2i3teMYv0FxEYQ6';
  if (!apiKey) throw new Error('OPENCLICKY_ELEVENLABS_API_KEY not set');
