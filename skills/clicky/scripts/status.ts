...
export function status(): StatusReport {
  ...
  return {
    ...
    voice: {
      say: isDarwin(),
      vibevoice: {
        configured: Boolean(process.env.CLICKY_VIBEVOICE_URL?.trim()),
        url: process.env.CLICKY_VIBEVOICE_URL?.trim() || null,
        reachable: false,
      },
      elevenlabs: { configured: Boolean(process.env.CLICKY_ELEVENLABS_API_KEY?.trim()) },
    },
