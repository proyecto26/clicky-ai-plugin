...
const USAGE = `clicky-ai — screen-aware Claude Code companion for macOS
...
subcommands:
  install [--force] [--dry-run]
      install /Applications/Clicky.app via Homebrew tap (preferred) or
      GitHub Releases DMG (fallback).

  launch
      launch the installed Clicky.app. Errors cleanly if not installed.
  ...
  speak "text" [--engine say|vibevoice|elevenlabs] [--voice NAME] [--rate N]
      ...
      CLICKY_VIBEVOICE_URL, ElevenLabs via CLICKY_ELEVENLABS_API_KEY.
